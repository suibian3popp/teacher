<template>
  <div style="margin:auto;text-align: center;font-size: 18px;font-weight: bold;">
    编辑信息
  </div>

  <el-form
    label-width="auto"
    :model="userInfo"
    style="max-width:600px;margin:auto;">

    <el-form-item label="昵称" >
      <el-input v-model="userInfo.nick" />
    </el-form-item>

    <el-form-item label="密码" >
      <el-input v-model="userInfo.password" />
    </el-form-item>

    <el-form-item label="性别" >
      <el-input v-model="userInfo.sex" />
    </el-form-item>

    <el-form-item label="手机" >
      <el-input v-model="userInfo.phone" />
    </el-form-item>

    <el-form-item label="邮箱" >
      <el-input v-model="userInfo.email" />
    </el-form-item>

    <el-form-item label="住址" >
      <el-input v-model="userInfo.address" />
    </el-form-item>

<!--    上传头像-->
    <el-form-item label="上传头像">
      <el-upload
          ref="uploadImageRef"
          class="avatar-uploader"
          accept=".jpg,.jpeg,.png"
          limit="1"
          :on-success="imageSuccess"
          :on-error="imageError"
          :file-list="imageFileList"
          :action="`http://localhost:8090/api/user/image?id=${id}`"
          :auto-upload="false"
          :show-file-list="true">
       <img v-if="imageUrl" :src="imageUrl" class="avatar"  alt=""/>
        <el-icon v-else class="avatar-uploader-icon"><Plus /></el-icon>
      </el-upload>
<!--      插值表达式-->
      {{imageTip}}
    </el-form-item>

<!--    上传合同-->
    <el-form-item label="上传合同">
      <el-upload
          ref="uploadContractRef"
          class="avatar-uploader"
          accpet="application/pdf"
          limit="1"
          :on-success="contractSuccess"
          :on-error="contractError"
          :file-list="contractFileList"
          :action="`http://localhost:8090/api/user/contract?id=${id}`"
          :auto-upload="false"
          :show-file-list="true"
          :before-upload="beforeContractUpload"
          @success="handleContractSuccess">
<!--        <img v-if="imageUrl" :src="imageUrl" class="avatar"  alt=""/>-->
        <div v-if="contractUrl" class="contract-preview">
          <span class="el-icon-document"></span>
          <span>{{ contractUrl.split('/').pop() || '合同文件' }}</span>
        </div>
        <el-icon v-else class="avatar-uploader-icon"><Plus /></el-icon>
      </el-upload>
      {{contractTip}}
    </el-form-item>

    <el-form-item label="&nbsp">
      <el-button type="primary" @click="submitUpload">提交</el-button>
      <el-button type="success" plain @click="goBack">返回</el-button>
    </el-form-item>

  </el-form>

</template>


<script setup>
import {onMounted, ref} from "vue";
import {Plus} from "@element-plus/icons-vue";
import {useRoute, useRouter} from "vue-router";
import {doGet, doPut} from "@/http/httpRequest";
import {ElMessage} from "element-plus";

let userInfo = ref({})
let uploadImageRef=ref(null)
let uploadContractRef = ref(null)
const imageUrl = ref(''); // 定义变量，初始值为空

let imageFileList=ref([])
let contractFileList=ref([])
let imageTip=ref("")
let contractTip=ref("")

//获取动态路由中的id参数
let route=useRoute()
let id=route.params.id

//调用路由方法
let router=useRouter()

onMounted(()=>{
  //根据id查询用户信息
  doGet("api/user/"+id,{}).then((resp)=>{
    if(resp.data.code===200){
      userInfo.value = resp.data.data;
    }
  })
})

// 提交触发按钮
function submitUpload() {
  //提交用户信息
  let formData = new FormData();
  for (let field in userInfo.value) {
    if (field !== 'userImageDO' && field !== 'userContractDO') {
      if (userInfo.value[field]) {
        formData.append(field, userInfo.value[field]);
      }
    }

  }
  doPut("/api/user", formData).then(resp => {
    if (resp.data.code === 200) {
      console.log("更新成功......")
    }
  })

  uploadImageRef.value.submit()
  uploadContractRef.value.submit()
}

//上传成功/失败
function imageSuccess() {
  imageFileList.value = []
  imageTip = "头像上传成功"
}
function imageError(){
  imageFileList.value=[]
  imageTip = "头像上传失败"
}
function contractSuccess(){
  contractFileList.value=[]
  contractTip="合同上传成功"
}
function contractError(){
  contractFileList.value=[]
  contractTip="合同上传失败"
}

function goBack(){
  router.go (-1)
}

//检验合同上传文件
const beforeContractUpload = (file) => {
  // 校验文件扩展名
  const isPdf = file.name.toLowerCase().endsWith('.pdf');

  if (!isPdf) {
    ElMessage.error('请上传 PDF 格式的文件！');
    return false; // 阻止上传
  }

  // 校验文件大小（可选）
  const isLt10M = file.size / 1024 / 1024 < 1000;
  if (!isLt10M) {
    ElMessage.error('文件大小不能超过 1000MB！');
    return false;
  }

  return true;
};

</script>


<!--scoped指的是局部样式-->
<style scoped>
.avatar-uploader .avatar {
  width: 178px;
  height: 178px;
  display: block;
}
</style>

<style>
.avatar-uploader .el-upload {
  border: 1px dashed var(--el-border-color);
  border-radius: 6px;
  cursor: pointer;
  position: relative;
  overflow: hidden;
  transition: var(--el-transition-duration-fast);
}

.avatar-uploader .el-upload:hover {
  border-color: var(--el-color-primary);
}

.el-icon.avatar-uploader-icon {
  font-size: 28px;
  color: #8c939d;
  width: 178px;
  height: 178px;
  text-align: center;
}
</style>